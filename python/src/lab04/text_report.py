...
stdpath = {
    "--in": ["/home/wilex/Документы/GitHub/python_labs/python/data/lab04/input.txt"],
    "--out": "/home/wilex/Документы/GitHub/python_labs/python/data/lab04/report.csv",
    "--per-file": "/home/wilex/Документы/GitHub/python_labs/python/data/lab04/report_per_file.csv",
}
...
input_f, output_f, per_f = (
    (
        stdpath["--in"]
        if not input_f
        else (
            input_f
            if all(os.path.isdir(os.path.dirname(x)) for x in input_f)
            else [mainpath + x for x in input_f]
        )
    ),
    (
        stdpath["--out"]
        if not output_f
            ...
            output_f
            if os.path.isdir(os.path.dirname(output_f))
            else mainpath + output_f
        )
    ),
    (
        stdpath["--per-file"]
        if not per_f
        else per_f if os.path.isdir(os.path.dirname(per_f)) else mainpath + per_f
    ),
)
